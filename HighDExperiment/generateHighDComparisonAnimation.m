function generateHighDComparisonAnimation(varargin)

    if nargin == 7
        [params, vizHistory, bestMatchTraj, scene_num, which_side, startFrame, endFrame]    = deal(varargin{:});
        outfile     = sprintf('./HighDExperiment/HighDResults/highD_%s_%s_%d_%d_comp.gif', scene_num, which_side, startFrame, endFrame);
    else
        [params, vizHistory, bestMatchTraj, startFrame, endFrame, outfile]   = deal(varargin{:});
    end
    ...
    for frame_i = startFrame:endFrame
        ...
        % =================================================================
        % Plot HighD Predicted Trajectory
        % =================================================================
        for v_i = 1:num_vecs
            if isempty(bestMatchTraj{v_i})
                continue;
            end
            state   = bestMatchTraj{v_i}(:, frame_i);
            if isequal(state, [-inf; 0; 0]) || isempty(vizHistory{frame_i, v_i})
                continue;
            end
            l_car   = params.vehiclesDims(1, v_i);
            w_car   = params.vehiclesDims(2, v_i);
            ...
            rectangles  = [ x-l_car/2, y+w_car/2; x-l_car/2, y-w_car/2;
                            x+l_car/2, y+w_car/2; x+l_car/2, y-w_car/2;
                            x+(l_car/2-1), y+w_car/2; x+(l_car/2-1), y-w_car/2];
            x_rect  = [rectangles(2,1), rectangles(1,1), rectangles(3,1), rectangles(4,1)];
            y_rect  = [rectangles(2,2), rectangles(1,2), rectangles(3,2), rectangles(4,2)];
            fill(x_rect, y_rect, 'k', 'FaceAlpha', 0.3, 'EdgeColor', 'k', 'LineStyle', '--');
            text(x-0.4*l_car, y, [num2str(v_i)]);
        end
        % =================================================================
        ...
        annotation('textbox', [0.133 0.8 0.20 0.12],...
            'String', sprintf('t = %0.2f s    (Frame %d)', (frame_i-1)*dt, frame_i),...
